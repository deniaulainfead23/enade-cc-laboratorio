...
2. Configure os três secrets de GitHub `SUPABASE_ACCESS_TOKEN`, `SUPABASE_DB_PASSWORD` e `SUPABASE_PROJECT_ID`; depois execute **Actions → Aplicar migrações Supabase → Run workflow** para criar o esquema. O passo a passo está em [`docs/04-instalacao-publicacao-automacao.md`](docs/04-instalacao-publicacao-automacao.md). Se preferir o SQL Editor, execute `supabase/schema.sql` uma única vez e não rode também a migração inicial automatizada.
